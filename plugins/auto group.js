...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!botAdmin) return m.reply("*🧭 "Administrator permissions are required for the bot to run this command."*");
    if (!senderAdmin) return m.reply("*🔐 "Administrator privileges are required to use this command.*");
